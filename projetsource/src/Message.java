...
public class Message {
	...
	public boolean  good=true; //sera modifie si le message n'est pas au bon format
	
	public Message(String msg){
		if (msg != null) {
			String header="";
			String options="";
			if(msg.length()>=12){//un message sans option nin data ne sert à rien
				header=msg.substring(0,11);
				datalength = (256 * msg.charAt(1) + msg.charAt(0));//on recupere la datalength;
				if(datalength>0){data=msg.substring(msg.length()-datalength);}else{data="";};
				traiterHeader(header);
				if(datalength>0){options=msg.substring(11,msg.length()-datalength);}else{options=msg.substring(11);}
