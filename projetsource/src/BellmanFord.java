...
/*
  When this timer expires, then one of our
  1-step neighbours hasn't sent HELLO for 
  some times, so we remove it from our table
 */
...
/*
 This timer advertises periodically
 HELLO msg to our 1-step neighbours
...
/*
 Just a debugging timer which
 advertises on the local console the
...
class BellmanFord {
	PhysicalLayer pl;
	String myID;
	/* the routing table : t = table.get(toto) gives us every path known
     to reach toto  t.get(tutu) gives us the length of the path to 
     reach toto via tutu

     Note: since there is hashtable manipulation called by a timer, it's safer
     to use a concurrent hashmap to avoid accessing the table by two threads at
     the same time
	 */
	...
	/* this method computes the String which represents the
         ALL:me:VECTOR:n:x1:y1:l1:....:xn:yn:ln
     message
	 */
	...
	String preProcessVector(){
		/* + on String is really inefficient and should be avoid (especially when loops
       are involved
		 */ 
	...
	/* for debugging purpose, compute the distance vector
     and print it on the local console 
	 */
	void printVector(){
	...
	void addEntry(String who,String by,int dist){
		...
		if(!table.containsKey(who)){
			...
			ConcurrentHashMap<String,Integer> step = new 
			ConcurrentHashMap<String,Integer>(20);
			step.put(by,new Integer(dist));
			table.put(who,step);
			processVector();
		}else{
			/* This case should happen when one of my neighbour has died, and
         is back live again. 
			 */
	...
	void removeEntry(String who){
		if(table.containsKey(who)){
			...
			for(Map.Entry<String,ConcurrentHashMap<String,Integer>> e : table.entrySet()){
				...
				// ... has to be removed
				steps.remove(who);
				// we remove empty sets of path from our table to avoid "x:::" in our distance vector
				if(steps.size() == 0){
					table.remove(dest);
				}
			}
...
//		System.out.println("Received hello from :"+from);
		if(!helloTable.containsKey(from)){
			...
			t.schedule(hellotask,10000);
			helloTable.put(from,t);
			/* We a close neighbours dies, we may still have some other 
         path to reach it. When it comes back alive, we need to update
         our routing table
			 */
	...
	boolean handleVector(String from,StringTokenizer st){
		...
		for(int i = 0; i < num; i++){
			...
			// not usefull for the standard algoirthm but nice for looping detection
			String by =  st.nextToken();	
			int length = Integer.parseInt(st.nextToken()); // the length to reach dest
			if(dest.equals(myID)){
				// we always know how to reach ourself
				continue;
			}
			if(!table.containsKey(dest)){
				// if we don't know how to reach this destination : create the entry
...
//					System.out.println("DEBUG msg : "+msg);
					...
					if(dest.equals(ID)){
						if(from.equals(ID)){//je suis l envoyeur de ce message
							continue;
						}else if(dest.equals(ID)){//si je suis le destinataire
							msg.from=ID;
						...
						if(typ.equals("hello")){
							...
							//System.out.println("VECTOR RECIEVED from "+ from);
							//router.handleVector(from,str);
						}
					}else if(! from.equals(ID)){//je ne suis pas destinataire et le message ne me concerne pas
						msg.from=ID;
						TraiterOptions to = new TraiterOptions();
						to.OPTMAP=msg.opt;
						to.traiterOptions();
						msg.opt=to.OPTMAP;
						pl.send(msg);
						System.out.print("NON DEST RENVOI : ");
						msg.print();
						continue;
					}
				}
